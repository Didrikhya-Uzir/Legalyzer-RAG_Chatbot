...
def initialize_session_state():
    ...
    if "conversation" not in st.session_state:
        ...
        index_name = "legal-db"  # Replace with the name of your Pinecone index

        # Retrieve existing indexes
        index_list = pc.list_indexes()
        existing_indexes = [index for index in index_list]
        
        # Check if the index exists before using it
        if index_name not in existing_indexes:
            raise ValueError(f"Pinecone index '{index_name}' not found. Please create it in the Pinecone console.")

        docsearch = LangPinecone.from_index_name(index_name=index_name, embedding=embeddings)
...
with prompt_placeholder:
    st.markdown("**Chat**")
    cols = st.columns((6, 1))
    cols[0].text_input("Chat", label_visibility="collapsed", key="human_prompt")
